...
bool PF::RawHistogramArea::on_expose_event (GdkEventExpose * event)
{
  ...
  PF::RawDeveloperConfigGUI::RawDeveloperConfigGUI( PF::Layer* layer ): OperationConfigGUI( layer, "Raw Developer" ),
      ...
      hotp_enable_checkbox( this, "hotp_enable", "", false ),
      hotp_enable_label(_("hot pixels removal")),
      ...
      hotp_frame(),
      ca_mode_selector( this, "tca_method", _("CA correction: "), PF::PF_TCA_CORR_AUTO, 100 ),
      ...
      demoMethodSelector( this, "demo_method", _("demosaicing: "), PF::PF_DEMO_AMAZE, 200 ),
      ...
      hlrecoModeSelector( this, "hlreco_mode", _("highlights reco: "), PF::HLRECO_CLIP, 200 ),
      profileModeSelector( this, "profile_mode", _("type: "), 1, 200 ),
      ...
      gammaModeSelector( this, "gamma_mode", "raw curve: ", 0, 200 ),
      inGammaLinSlider( this, "gamma_lin", "Gamma linear", 0, 0, 100000, 0.05, 0.1, 1, 250, 3),
      inGammaExpSlider( this, "gamma_exp", "Gamma exponent", 2.2, 0, 100000, 0.05, 0.1, 1, 250, 3),
      outProfileModeSelector( this, "out_profile_mode", _("type: "), 1, 200 ),
      outProfileTypeSelector( this, "out_profile_type", _("gamut: "), 1, 200 ),
      outTRCTypeSelector( this, "out_trc_type", _("encoding: "), 1, 200 ),
      ...
      ignore_temp_tint_change( false )
  {
    ...
    //=====================
    // white balance
    wbModeSelector.set_tooltip(_("choose white balance preset from camera"));
    ...
    wbTempSlider.set_tooltip(_("adjust the color temperature in Kelvin degrees"));
    wbTintSlider.set_tooltip(_("adjust the color tint from magenta to green"));
    ...
    for( unsigned int i = 0; i < PF::WB_LAST; i++ ) {
      ...
      wbRedSliders[i]->set_tooltip(_("adjust the gain of the red channel"));
      wbGreenSliders[i]->set_tooltip(_("adjust the gain of the green channel"));
      wbBlueSliders[i]->set_tooltip(_("adjust the gain of the blue channel"));
    ...
    //=====================
    // exposure
    exposureSlider.set_tooltip(_("adjust the exposure correction"));
    hlrecoModeSelector.set_tooltip(_("choose the highlights reconstruction method (clip, blend or none)"));
    ...
    //=====================
    // demosaicing
    demoMethodSelector.set_tooltip(_("choose the raw data demosaicing method"));
    fcsSlider.set_tooltip(_("set the number of false color correction steps"));
    ...
    blackLevelRSlider.set_tooltip(_("adjust the black level for the red channel"));
    blackLevelG1Slider.set_tooltip(_("adjust the black level for the first green channel"));
    blackLevelG2Slider.set_tooltip(_("adjust the black level for the second green channel"));
    blackLevelBSlider.set_tooltip(_("adjust the black level for the blue channel"));
    saturationLevelSlider.set_tooltip(_("adjust the white level for all channels"));
    ...
    hotp_enable_checkbox.set_tooltip(_("enable/disable the hot pixels removal filter"));
    hotp_threshold_slider.set_tooltip(_("adjust the hot pixels detection threshold"));
    hotp_strength_slider.set_tooltip(_("adjust the strength of the hot pixels filter"));
    hotp_permissive_checkbox.set_tooltip(_("make the filter more permissive"));
    ...
    hotp_enable_hbox.pack_start(hotp_enable_checkbox);
    hotp_enable_hbox.pack_start(hotp_enable_label);
    hotp_enable_hbox.set_spacing(2);
    hotp_frame.set_label_widget(hotp_enable_hbox);


    //=====================
    // lens distortions corrections
    ...
    lf_auto_matching_checkbox.set_tooltip(_("select cmaera/lens model based on EXIF data"));
    lf_enable_all_button.set_tooltip(_("enable all available corrections"));
    lf_auto_crop_checkbox.set_tooltip(_("automatically crop the image to remove outer black bands"));
    lf_enable_vignetting_button.set_tooltip(_("enable the vignetting correction"));
    lf_enable_distortion_button.set_tooltip(_("enable the barrel distortion correction"));
    lf_enable_tca_button.set_tooltip(_("enable the Transverse Chromatic Aberration (TCA) correction"));
    ca_mode_selector.set_tooltip(_("choose the chromatic aberration correction mode\n* profiled: use the lensfun database\n* automatic: extract parameters directly from image"));
    ...
    profileModeSelector.set_tooltip(_("choose the camera color profile type:\n* raw color: turn off color management\n* standard profile: color profile with a neutral and reliable color rendering\n* ICC from disk: select an ICC profile from disk\n* DCP from disk: select a DCP profile from disk"));
    gammaModeSelector.set_tooltip(_("choose the type of tone curve to be applied before ICC conversion\n* linear: direct conversion\n* sRGB: apply a standard sRGB tone curve\n* custom: apply a power-like tone curve with a custom exponent"));
    inGammaExpSlider.set_tooltip(_("exponent for power-like tone curve before ICC conversion"));
    use_tone_curve_checkbox.set_tooltip(_("activate the tone curve in the DCP profile"));
    apply_hue_sat_map_checkbox.set_tooltip(_("apply the hue/saturation mapping"));
    apply_look_table_checkbox.set_tooltip(_("apply the output color lookup table"));
    apply_baseline_exposure_offset_checkbox.set_tooltip(_("apply the exposure offset"));
    outProfileModeSelector.set_tooltip(_("choose the type of output color profile"));
    outTRCTypeSelector.set_tooltip(_("choose the encoding of the output pixel values"));
    clip_negative_checkbox.set_tooltip(_("clip the negative output values to zero"));
    clip_overflow_checkbox.set_tooltip(_("clip the positive output values to unity"));
    camProfOpenButton.set_tooltip_text(_("open a camera input ICC profile from disk"));
    camProfFileEntry.set_tooltip_text(_("path to the camera input ICC profile from disk"));
    camDCPProfOpenButton.set_tooltip_text(_("open a camera input DCP profile from disk"));
    camDCPProfFileEntry.set_tooltip_text(_("path to the camera input DCP profile from disk"));
    ...
    camProfLabel.set_text( "ICC file name:" );
    ...
    camDCPProfLabel.set_text( "DCP file name:" );
  ...
  void PF::RawDeveloperConfigGUI::do_update()
  {
    ...
    if( inode && inode->image) {
      ...
      prop = par->get_property( "black_level_correction_g1" );
      if(prop) {
        float black;
        prop->get(black);
        histogramArea.black = black;
      } else
        histogramArea.black = 128;
      prop = par->get_property( "saturation_level_correction" );
      if(prop) {
        double white;
        prop->get(white);
        histogramArea.white = white_level_prop_to_slider(white, this, NULL);
      } else
    ...
    if( processor ) {
      ...
      if( par2 ) {
        ...
        if(par2->get_lf_match_found()) {
          lf_enable_distortion_button.set_sensitive(true);
          lf_enable_tca_button.set_sensitive(true);
          lf_enable_vignetting_button.set_sensitive(true);
          lf_enable_all_button.set_sensitive(true);
        } else {
          lf_enable_distortion_button.set_sensitive(false);
          lf_enable_tca_button.set_sensitive(false);
          lf_enable_vignetting_button.set_sensitive(false);
          lf_enable_all_button.set_sensitive(false);
        }
      }
    }
    /*
  if( par->get_all_enabled() ) {
    ...
    case PF::IN_PROF_ICC:
      ...
      gammaModeHBox.show();
      {
        PF::PropertyBase* prop2 = par->get_property( "gamma_mode" );
        if( prop2 && prop2->get_enum_value().first == PF::IN_GAMMA_CUSTOM )
          inGammaExpSlider.show();
        else
          inGammaExpSlider.hide();
      }
      camDCPProfHBox.hide();
      dcp_options_box.hide();
      outProfFrame.show();
