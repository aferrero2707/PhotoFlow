...
namespace PF
{
  ...
  class Layer
  {
    ...
    std::vector< std::pair< std::pair<int32_t,int32_t>,bool> > inputs;
  ...
  public:
    ...
    void set_input(int n, int32_t lid, int32_t imgid, bool blended=true) {
      for( int i = inputs.size(); i <= n; i++ )
        inputs.push_back( std::make_pair(std::make_pair((int32_t)-1,(int32_t)-1),(bool)true) );
      inputs[n].first.first = lid;
      inputs[n].first.second = imgid;
      inputs[n].second = blended;
    }
    void add_input(int32_t lid, int32_t imgid, bool blended=true)
    {
      inputs.push_back( std::make_pair(std::make_pair(lid,imgid), blended) );
    }
    void remove_input(int32_t lid);
    std::vector< std::pair< std::pair<int32_t,int32_t>,bool> >& get_inputs() { return inputs; }
