...
public:
  ...
  void rebuild_done_signal() { /*g_cond_signal( rebuild_done );*/ vips_semaphore_up( &rebuild_sem ); }
  void export_done_signal() { g_cond_signal( export_done ); }
  void sample_done_signal() { /*g_cond_signal( sample_done );*/  vips_semaphore_up( &sample_sem ); }
  ...
  void do_update( PF::Pipeline* pipeline=NULL, bool sync=false );
