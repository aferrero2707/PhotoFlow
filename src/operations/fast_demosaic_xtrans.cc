...
PF::FastDemosaicXTransPar::FastDemosaicXTransPar():
  OpParBase(), normalize( false )
{
  set_demand_hint( VIPS_DEMAND_STYLE_FATSTRIP );
  set_type( "fast_demosaic_xtrans" );
}


VipsImage* PF::FastDemosaicXTransPar::build(std::vector<VipsImage*>& in, int first,
				     ...
				     unsigned int& level)
{
  ...
  PF_UNREF( t[i0+1], "AmazeDemosaicXTransPar::build(): t[i0+1] unref #1" );
  ...
  PF_UNREF( t[i0+2], "AmazeDemosaicXTransPar::build(): t[i0+2] after vips_copy()" );
