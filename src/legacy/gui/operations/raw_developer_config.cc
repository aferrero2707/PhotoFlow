...
void PF::RawDeveloperConfigGUIV1::temp2mul(double TempK, double tint, double mul[3])
{
...
void PF::RawDeveloperConfigGUIV1::mul2temp(float coeffs[3], double *TempK, double *tint)
{
  //dt_iop_temperature_gui_data_t *g = (dt_iop_temperature_gui_data_t *)self->gui_data;
...
bool PF::WBSelectorV1::check_value( int id, const std::string& name, const std::string& val )
{
  if( id < 3 ) return true;
      ...
      ignore_temp_tint_change( false )
{
  ...
  camProfFileEntry.signal_activate().
      connect(sigc::mem_fun(*this,
          &RawDeveloperConfigGUIV1::on_cam_filename_changed));
  camProfOpenButton.signal_clicked().connect(sigc::mem_fun(*this,
      &RawDeveloperConfigGUIV1::on_cam_button_open_clicked) );

  outProfFileEntry.signal_activate().
      connect(sigc::mem_fun(*this,
          &RawDeveloperConfigGUIV1::on_out_filename_changed));
  outProfOpenButton.signal_clicked().connect(sigc::mem_fun(*this,
      &RawDeveloperConfigGUIV1::on_out_button_open_clicked) );

  wbTempSlider.get_adjustment()->signal_value_changed().
      connect(sigc::mem_fun(*this,&PF::RawDeveloperConfigGUIV1::temp_tint_changed));
...
void PF::RawDeveloperConfigGUIV1::temp_tint_changed()
{
  ...
  if( get_layer() && get_layer()->get_image() &&
    ...
    PF::RawDeveloperV1Par* par =
        dynamic_cast<PF::RawDeveloperV1Par*>(get_layer()->get_processor()->get_par());
    if( !par ) return;
...
void PF::RawDeveloperConfigGUIV1::do_update()
{
  std::cout<<"RawDeveloperConfigGUIV1::do_update() called."<<std::endl;
  if( get_layer() && get_layer()->get_image() && 
      get_layer()->get_processor() &&
      get_layer()->get_processor()->get_par() ) {
    PF::RawDeveloperV1Par* par =
    ...
    if( inode && inode->image) {
      ...
      if( !vips_image_get_blob( inode->image, PF_META_EXIF_NAME,(void**)&exif_data,&blobsz ) &&
        ...
        //std::cout<<"RawDeveloperConfigGUIV1::do_update(): maker="<<maker<<" model="<<model<<std::endl;
    ...
    //std::cout<<"PF::RawDeveloperConfigGUIV1::do_update() called."<<std::endl;
...
/*
void PF::RawDeveloperConfigGUIV1::pointer_press_event( int button, double x, double y, int mod_key )
{
  if( button != 1 ) return;
}
 */

bool PF::RawDeveloperConfigGUIV1::pointer_release_event( int button, double sx, double sy, int mod_key )
{
...
void PF::RawDeveloperConfigGUIV1::spot_wb( double x, double y )
{
  ...
  PF::RawDeveloperV1Par* par = dynamic_cast<PF::RawDeveloperV1Par*>( l->get_processor()->get_par() );
  if( !par ) return;
  ...
  PF::RawDeveloperV1Par* node_par = dynamic_cast<PF::RawDeveloperV1Par*>( node->processor->get_par() );
  if( !node_par ) return;
  ...
  for( int i = 0; i < 100; i++ ) {
    ...
    //std::cout<<"RawDeveloperConfigGUIV1: getting spot WB ("<<x<<","<<y<<")"<<std::endl;
    img->sample( l->get_id(), x, y, sample_size, NULL, values );
    if( values.size() != 3 ) {
      std::cout<<"RawDeveloperConfigGUIV1::pointer_relese_event(): values.size() "
    ...
    //std::cout<<"RawDeveloperConfigGUIV1: checking spot WB"<<std::endl;
    img->sample( l->get_id(), x, y, sample_size, NULL, values );
    if( values.size() != 3 ) {
      std::cout<<"RawDeveloperConfigGUIV1::pointer_relese_event(): values.size() "
          <<values.size()<<" (!= 3)"<<std::endl;
...
void PF::RawDeveloperConfigGUIV1::color_spot_wb( double x, double y )
{
  // Get the layer associated to this operation
  PF::Layer* l = get_layer();
  if( !l ) return;
  ...
  for( int i = 0; i < 100; i++ ) {
    ...
    std::cout<<"RawDeveloperConfigGUIV1: getting color spot WB"<<std::endl;
    /*
     ...
     */
    std::cout<<"RawDeveloperConfigGUIV1: getting color spot WB ("<<x<<","<<y<<")"<<std::endl;
    img->sample( l->get_id(), x, y, sample_size, NULL, values );
...
void PF::RawDeveloperConfigGUIV1::on_cam_button_open_clicked()
{
...
void PF::RawDeveloperConfigGUIV1::on_out_button_open_clicked()
{
  Gtk::FileChooserDialog dialog("Please choose a file",
      Gtk::FILE_CHOOSER_ACTION_OPEN);
  //dialog.set_transient_for(*this);
