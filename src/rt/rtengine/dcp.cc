...
#define safe_g_fopen g_fopen

// Color space conversion to/from XYZ; color spaces adapted to D50 using Bradford transform
static const double xyz_sRGB[3][3] = {{0.4360747,  0.3850649,  0.1430804},
    {0.2225045,  0.7168786,  0.0606169},
    {0.0139322,  0.0971045,  0.7141733}
};

static const double sRGB_xyz[3][3] = {{3.1338561, -1.6168667, -0.4906146},
    { -0.9787684,  1.9161415,  0.0334540},
    {0.0719453, -0.2289914,  1.4052427}
};

static void inverse33 (const double (*rgb_cam)[3], double (*cam_rgb)[3])
{
...
double DCPProfile::camwb_to_temp(const double cam_wb[3], int preferred_illuminant) const
{
...
DCPProfile::Matrix DCPProfile::makeXyzCam(double cam_wb[3], double camWbMatrix[3][3], int preferred_illuminant) const
{
    // Code adapted from dng_color_spec::FindXYZtoCamera.
    // Note that we do not support monochrome or colorplanes > 3 (no reductionMatrix support),
    // we do not support cameracalibration either.
    ...
    Triple neutral; // Same as the DNG "AsShotNeutral" tag if white balance is Camera's own
    {
      ...
      /* PhotoFlow already has WB multipliers in CameraRGB space
        double r, g, b;
...
void rtengine::hsdApply(const DCPProfile::HsdTableInfo& table_info, const std::vector<DCPProfile::HsbModify>& table_base, float& h, float& s, float& v)
{
    ...
    if (table_info.val_divisions < 2) {
        ...
        std::vector<DCPProfile::HsbModify>::size_type e00_index = h_index0 * table_info.pc.hue_step + s_index0;
        std::vector<DCPProfile::HsbModify>::size_type e01_index = e00_index + (h_index1 - h_index0) * table_info.pc.hue_step;
        ...
        std::vector<DCPProfile::HsbModify>::size_type e00_index = v_index0 * table_info.pc.val_step + h_index0 * table_info.pc.hue_step + s_index0;
        std::vector<DCPProfile::HsbModify>::size_type e01_index = e00_index + (h_index1 - h_index0) * table_info.pc.hue_step;
        std::vector<DCPProfile::HsbModify>::size_type e10_index = e00_index + table_info.pc.val_step;
        std::vector<DCPProfile::HsbModify>::size_type e11_index = e01_index + table_info.pc.val_step;
...
void DCPProfile::MakeXYZCAM(double cam_wb[3], double camWbMatrix[3][3], int preferredIlluminant, double (*mXYZCAM)[3]) const
{
    // code adapted from dng_color_spec::FindXYZtoCamera
    // note that we do not support monochrome or colorplanes > 3 (no reductionMatrix support)
    // we do not support cameracalibration either
