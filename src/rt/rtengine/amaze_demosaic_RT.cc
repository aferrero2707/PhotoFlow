...
#pragma omp for schedule(dynamic) collapse(2) nowait
    ...
    for (top=tiley-16; top < tiley+tileh-16; top += TS-32)
      for (left=tilex-16; left < tilex+tilew-16; left += TS-32) {
        ...
        if (bottom>(winy+height)) {rrmax=winy+height-tiley;} else {rrmax=rr1;}
        if (right>(winx+width)) {ccmax=winx+width-tilex;} else {ccmax=cc1;}

        // Offset of RAW data with respect to the top/left corner of the current tile
        FC_roffset = rrmin;
        FC_coffset = ccmin;
        /*
