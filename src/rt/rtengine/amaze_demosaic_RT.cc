...
SSEFUNCTION void RawImageSource::amaze_demosaic_RT(int winx, int winy, int winw, int winh, int tilex, int tiley, int tilew, int tileh)
{
    BENCHFUN
    ...
    for (top=tiley-16; top < tiley+tileh-16; top += ts-32) {
      for (left=tilex-16; left < tilex+tilew-16; left += ts-32) {
        ...
        ntilesprocessed += 1;
                ...
                int bottom = min(top + ts, tiley + tileh + 16);
                //location of tile right edge
                //int right  = min(left + ts, winx + width + 16);
                int right  = min(left + ts, tilex + tilew + 16);
                ...
                int rrmax = bottom > (winy + height) ? winy + height - tiley : rr1;
                int ccmax = right > (winx + width) ? winx + width - tilex : cc1;
                ...
                for (int rr = 10; rr < rr1 - 10; rr++)
                    ...
                    for (int cc = 10 + (FC(rr, 2) & 1), indx = rr * ts + cc, indx1 = indx >> 1; cc < cc1 - 10; cc += 2, indx += 2, indx1++) {
                        ...
                        if (fabs(0.5 - pmwt[indx1]) < fabs(0.5 - pmwtalt)) {
                ...
                for (int rr = 12; rr < rr1 - 12; rr++)
                    ...
                    for (int cc = 12 + (FC(rr, 2) & 1), indx = rr * ts + cc, indx1 = indx >> 1; cc < cc1 - 12; cc += 2, indx += 2, indx1++) {
                        ...
                        if (fabs(0.5 - pmwt[indx >> 1]) < fabs(0.5 - hvwt[indx >> 1]) ) {
                            continue;
                        }
                ...
                for (int rr = 16; rr < rr1 - 16; rr++) {
                    ...
                    if((FC(rr, 2) & 1) == 1) {
                        for (; indx < rr * ts + cc1 - 16 - (cc1 & 1); indx++, col++) {
                            float temp =  1.f / (hvwt[(indx - v1) >> 1] + 2.f - hvwt[(indx + 1) >> 1] - hvwt[(indx - 1) >> 1] + hvwt[(indx + v1) >> 1]);
                            red[row][col] = (rgbgreen[indx] - ((hvwt[(indx - v1) >> 1]) * Dgrb[0][(indx - v1) >> 1] + (1.f - hvwt[(indx + 1) >> 1]) * Dgrb[0][(indx + 1) >> 1] + (1.f - hvwt[(indx - 1) >> 1]) * Dgrb[0][(indx - 1) >> 1] + (hvwt[(indx + v1) >> 1]) * Dgrb[0][(indx + v1) >> 1]) *
                                                        temp);
                            blue[row][col] = (rgbgreen[indx] - ((hvwt[(indx - v1) >> 1]) * Dgrb[1][(indx - v1) >> 1] + (1.f - hvwt[(indx + 1) >> 1]) * Dgrb[1][(indx + 1) >> 1] + (1.f - hvwt[(indx - 1) >> 1]) * Dgrb[1][(indx - 1) >> 1] + (hvwt[(indx + v1) >> 1]) * Dgrb[1][(indx + v1) >> 1]) *
                                                         temp);

                            indx++;
                            col++;
                            red[row][col] = (rgbgreen[indx] - Dgrb[0][indx >> 1]);
                            blue[row][col] = (rgbgreen[indx] - Dgrb[1][indx >> 1]);
                        }
                ...
                for (int rr = 16; rr < rr1 - 16; rr++) {
                    ...
                    for (; cc < cc1 - 16; cc++) {
                        green[row][cc + left] = rgbgreen[rr * ts + cc];
